{ config, pkgs, lib, discoveryPsk ? "CHANGE_ME_TO_YOUR_PSK", discoveryServiceIp ? "192.168.1.100", configRepoUrl ? "github:yourusername/nixos-pi-configs", ... }:
{
  ...
  # Memory and performance optimizations for Pi
  boot.kernel.sysctl = {
    "vm.swappiness" = 10;
    "vm.vfs_cache_pressure" = 50;
    "kernel.panic" = 10;  # Reboot after kernel panic
  };
  ...
  nix = {
    ...
    settings = {
      ...
      max-jobs = 1;
      cores = 1;
      # Force binary substitutes to prevent compilation
      substituters = [
      ...
      trusted-public-keys = [
        ...
        "cache.garnix.io:CTFPyKSLcx5RMJKfLo5EEPUObbCHytdhIQacXkNZ8pU="
      ];
      # Prevent building from source - only use binary cache
      builders-use-substitutes = true;
      require-sigs = false;  # Allow unsigned packages temporarily
      auto-optimise-store = true;
      # Limit build cores to prevent memory issues
      build-cores = 1;
    };
  ...
  # LIGHTWEIGHT: Only essential packages for bootstrap - defer heavy packages to final config
  environment.systemPackages = with pkgs; [
    ...
    # Minimal network tools only - defer heavy tools to final config
    tcpdump
    netcat-gnu
    nmap
    iftop
  ...
  # Add swap to handle memory pressure
  swapDevices = [{
  ...
  environment.etc."bootstrap/bootstrap.sh" = {
    text = ''
      ...
      check_memory() {
          ...
          log_info "💾 Memory: ''${used_mem}MB/''${total_mem}MB used (''${usage_percent}%)"

          if [ $usage_percent -gt 85 ]; then
              log_warn "⚠️  High memory usage detected: ''${usage_percent}%"
              return 1
          fi
          return 0
      }
      ...
      monitor_system_resources() {
          ...
          df -h /boot | tail -1 | awk '{print "🥾 Boot: " $3 "/" $2 " used (" $5 ")"}'
          check_memory
          log_info "🌡️  CPU Load: $(uptime | awk -F'load average:' '{print $2}')"
      }
      ...
      main() {
          log_info "🚀 Starting Raspberry Pi Discovery Bootstrap Process (Lightweight)"
          log_info "================================================================="
          ...
          # Monitor system resources before rebuild
          monitor_system_resources

          # Apply NixOS configuration with memory safeguards
          log_info "📦 Applying NixOS configuration (optimized for low memory)..."
          ...
          # Ensure swap is available before rebuild
          if [ ! -f /var/lib/swapfile ]; then
              log_info "💾 Creating swap file for memory protection..."
              dd if=/dev/zero of=/var/lib/swapfile bs=1M count=2048 || true
              chmod 600 /var/lib/swapfile || true
              mkswap /var/lib/swapfile || true
              swapon /var/lib/swapfile || true
          fi
          ...
          if nixos-rebuild switch \
              ...
              --option trusted-public-keys "cache.nixos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY= cache.garnix.io:CTFPyKSLcx5RMJKfLo5EEPUObbCHytdhIQacXkNZ8pU=" \
              --option max-jobs 1 \
              --option cores 1 \
  ...
  systemd.services.pi-bootstrap = {
    description = "Raspberry Pi Discovery Bootstrap Process (Lightweight)";
    ...
    serviceConfig = {
      ...
      TimeoutStartSec = "1200";  # 20 minute timeout for slow rebuilds
      # Memory protection
      MemoryMax = "1.5G";
      MemorySwapMax = "2G";
      OOMScoreAdjust = -100;  # Protect from OOM killer
      Environment = [
    ...
    script = ''
      echo "Starting lightweight discovery bootstrap process..."
