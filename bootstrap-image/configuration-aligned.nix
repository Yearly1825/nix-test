...
{
  # Use SAME nixpkgs version as final config (nixos-24.05)
  system.stateVersion = "24.05";

  # Disable problematic assertions (same as final config)
  assertions = lib.mkForce [];

  # Boot configuration (aligned with final config)
  boot = {
    loader = {
      grub.enable = false;
      generic-extlinux-compatible.enable = true;
    };
    kernelPackages = pkgs.linuxPackages_rpi4;  # Same as final config
  ...
  # Hardware settings (same as final config)
  hardware = {
  ...
  # Explicit file system configuration (same as final config)
  fileSystems."/" = {
  ...
  # Enable NetworkManager (same as final config)
  ...
  # MINIMAL packages matching final config + discovery dependencies
  environment.systemPackages = with pkgs; [
    # Final config packages
    curl
    wget
    git
    htop
    ...
    # Minimal discovery service dependencies
    jq
    python3
  ...
  environment.etc."bootstrap/bootstrap.sh" = {
    text = ''
      ...
      main() {
          log_info "🚀 Starting Raspberry Pi Discovery Bootstrap Process (Aligned)"
          log_info "============================================================="
          ...
          # Apply NixOS configuration (should be minimal diff now)
          log_info "📦 Applying aligned NixOS configuration..."
          ...
          # Use same nixpkgs version and minimal build options
  ...
  # UPDATED: Improved systemd service
  systemd.services.pi-bootstrap = {
    description = "Raspberry Pi Discovery Bootstrap Process (Aligned)";
    ...
    script = ''
      echo "Starting aligned discovery bootstrap process..."
